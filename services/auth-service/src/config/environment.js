...
const ENV_SCHEMA = [
  ...
  { key: 'PORT',                    required: true,  validate: (v) => Number.isInteger(+v) && +v > 0 },
  ...
  { key: 'JWT_ALGORITHM',           required: true,  validate: (v) => ['HS256','HS384','HS512'].includes(v) },

  // Bcrypt
  { key: 'BCRYPT_ROUNDS',           required: true,  validate: (v) => +v >= 10 && +v <= 15 },
...
function validateEnvironment() {
  ...
  for (const { key, required, validate } of ENV_SCHEMA) {
    ...
    if (validate && !validate(value)) {
      errors.push(`  ✗ ${key}: valor inválido → "${value.substring(0, 30)}..."`);
    }
  }

  if (errors.length > 0) {
    logger.error('Configuración de entorno inválida. Proceso terminado.\n' + errors.join('\n'));
    process.exit(1);
  }
...
module.exports = {
  ...
  RATE_LIMIT_LOGIN_WINDOW_MS: parseInt(process.env.RATE_LIMIT_LOGIN_WINDOW_MINUTES || '15', 10) * 60_000,
